# Deployment Guide — Double J Gulf Services

## 1. Backend Deployment (Render)

1. Push backend folder to GitHub
2. Go to https://render.com
3. Create a new Web Service
4. Select your backend repo
5. Set:
   - Build Command: `npm install`
   - Start Command: `node server.js`
6. Add environment variables:
   - `MONGO_URI=your_atlas_connection_string`
   - `JWT_SECRET=your_secret_key`
7. Deploy

Render will give you a URL like:
https://doublej-backend.onrender.com

---

## 2. Frontend Deployment (Netlify)

1. Push frontend folder to GitHub
2. Go to https://netlify.com and sign in
3. Add New Site > Import from Git and select your frontend repo
4. Build command: `npm run build`, Publish directory: `dist`
5. Add environment variables:
   ```
   VITE_API_URL=https://doublej-backend.onrender.com
   ```
6. Click "Deploy Site"
7. After deploy, update your backend's CORS config to allow your Netlify URL!

---

## 3. MongoDB Atlas Setup

1. Go to https://cloud.mongodb.com
2. Create a free cluster
3. Add IP whitelist: 0.0.0.0/0
4. Create DB user & password
5. Copy connect string
6. Paste into Render’s MONGO_URI

---

## 4. Post-Deployment Checklist

- Test API endpoints
- Test customer and admin login
- Submit a test order
- Update order status
- Verify SEO metadata
- Verify sitemap + robots.txt
